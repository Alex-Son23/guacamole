...
class Profile:
    ...
    def __init__(self, phone: str, proxy_data: dict, query: str, user_agent: str = "") -> None:
...
class ProfileService:
    ...
    def use_profile(id: int) -> Profile:
        with open("data.json", "r") as f:
            all_accs = json.loads(f)
        acc = all_accs[id]
        if not acc["user-agent"]:
            acc["user-agent"] = fake_useragent.UserAgent(platforms="mobile").random
        with open("data.json", "w") as f:
            json.dumps(all_accs, f)
        profile = Profile(acc["phone"], acc["proxy"], acc["user-agent"])
        return profile
...
class RegisterService:
    ...
    def create_one(self, id: int):
        requester = self.profile_service.use_profile(id)
        resp = requester.client.post(AUTH_URL)
        data = requester.post("register")
        return 
